...
def get_fi_nas(a, h, l2, x1_aim, x1, x2, x3, x6):
    return (a[2] * x1)**-1 * (-h**-1 * x1_aim * (1 + l2) + (a[0] + a[1]*x6)*x2) - a[2]**-1 *(a[3] - h**-1 * (1 + l2) + a[4]*(1 - x2 - x3))


def get_fi_adar(a, h, l2, x1_aim, x1, x2, x3, x6):
    return (-a[2] * x1) ** -1 * (
                h ** -1 * (x1_aim - l2 * get_psi(x1_aim, x1) - x1) - (a[0] + a[1] * x6) * x2 + a[3] * x1 + a[4] * (
                    1 - x2 - x3) * x1)
...
def nas(a, c, psi1_prev, h, l1, l2, xi, x1_const, x, xlag):
    psi_temp = get_psi(x1_const, x[0][-1])
    fi_temp = get_fi_nas(a, h, l2, x1_const, x[0][-1], x[1][-1], x[2][-1], x[5][-1])
    psi1_temp = x[9][-1] - fi_temp
    psi2_temp = 0

    fi_next = get_fi_nas(a, h, l2, x1_const,
                         euler.x1_next(a, h, x),
                         euler.x2_next(a, h, x),
                         euler.x3_next(a, h, x),
                         euler.x6_next(a, xi, h, x, xlag))

    u_temp = h**-1 * (fi_next - l1*psi1_temp - x[9][-1]) - model.f10(a, x) - c * ((psi1_temp - l1*psi1_prev) / h)
    return psi_temp, psi1_temp, psi2_temp, u_temp
