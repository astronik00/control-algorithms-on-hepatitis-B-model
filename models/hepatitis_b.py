...
def f6(a, xi, x, xlag):
    return a[18] * xi * xlag[1][3][-1] * xlag[1][4][-1] * xlag[1][5][-1] - a[20] * x[3][-1] * x[4][-1] * x[5][-1] - a[
        21] * x[1][-1] * x[5][-1] - a[22] * x[3][-1] * x[5][-1] + a[23] * (1 - x[5][-1])


def f7(a, xi, x, xlag):
    return a[13] * xi * xlag[2][3][-1] * xlag[2][6][-1] - a[15] * x[3][-1] * x[6][-1] - a[16] * x[3][-1] * x[6][-1] * \
        x[7][-1] + a[17] * (1 - x[6][-1])


def f8(a, xi, x, xlag):
    return a[24] * xi * xlag[3][3][-1] * xlag[3][6][-1] * xlag[3][7][-1] - a[26] * x[3][-1] * x[6][-1] * x[7][-1] + a[
        27] * (1 - x[7][-1])
